    ...
    if (n == 3 && kw[0] == KY_ALPHA && kw[1] == KY_EQUAL) {
        ...
        if (block[blockIndex].type == BK_IF) {
            if (block[blockIndex].elseLine != 0) {
                cpJMP(block[blockIndex].startLine + 1, block[blockIndex].elseLine + 1,
                      OP_JNMP); // jump to else at startpos
                cpJMP(block[blockIndex].elseLine, block[blockIndex].endLine,
                      OP_JMP); // jump to end at else
            } else {
                cpJMP(block[blockIndex].startLine + 1, block[blockIndex].endLine,
                      OP_JNMP); // jump to end pos
            }
            bkPop();
        } else if (block[blockIndex].type == BK_WHILE) {
            cpJMP(block[blockIndex].startLine + 1, block[blockIndex].endLine + 1,
                  OP_JNMP); // jump out of loop
            cpJMP(block[blockIndex].endLine, block[blockIndex].startLine,
                  OP_JMP); // jump back to logic control
            PC++;
            bkPop();
        } else if (block[blockIndex].type == BK_FOR) {
            cpIncrease(block[blockIndex].forReg);
            cpJMP(block[blockIndex].startLine + 1, block[blockIndex].endLine + 2,
                  OP_JNMP); // jump out of loop
            cpJMP(block[blockIndex].endLine + 1, block[blockIndex].startLine,
                  OP_JMP); // jump out of loop
        }
